...
The HAL design (Section 11, Open Question #5) identified that transmitting 3 Sabertooth motor commands at the legacy 9600 baud took ~12.6 ms. Runtime now uses that 9600-baud baseline for the shared Sabertooth/SyRen bus because SyRen autobaud was not reliable at 19200 or 38400 on the current SoftwareSerial path.

**This is a hard physical constraint**: At runtime 9600 baud, one 4-byte packet takes ~4.2 ms to transmit. Multi-motor command bursts are blocking unless unchanged commands are suppressed, updates are staggered, or serial TX is moved out of the executor path.

**Impact on timing budget**: If motors update at 50 Hz (every 20 ms), each changed motor command costs about 4.2 ms at 9600 baud. Two drive packets cost about 8.3 ms; drive plus dome can cost about 12.5 ms. This must be explicitly modeled in the executor's timing analysis.
...
| DriveNode | 80 us | **4160-8330 us** (1-2 Sabertooth packets at 9600 baud) |
| DomeNode | 60 us | **4160 us** (1 Sabertooth packet at 9600 baud) |
| ServoNode | 40 us | **1560-7000 us** (Maestro speed or 11-channel batched command at 38400 baud) |
| AudioNode | 30 us | **1040-2080 us** (1-2 byte MP3 Trigger serial commands at 9600 baud) |

**Critical finding**: DriveNode, DomeNode, audio, and servo-command paths still exceed their original WCET budgets while using blocking SoftwareSerial. The rate-monotonic analysis in the safety design (Section 8.4) shows 3.9% CPU utilization, but this is based on incorrect microsecond-only WCET assumptions that omit blocking serial TX.
