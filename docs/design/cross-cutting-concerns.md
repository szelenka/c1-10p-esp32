...
The HAL design (Section 11, Open Question #5) identifies that transmitting 3 Sabertooth motor commands at 9600 baud takes ~12.6 ms. This exceeds even the 10 ms loop assumed by the HAL design, and grossly exceeds the 1 ms loop assumed by the other three designs.

**This is a hard physical constraint**: At 9600 baud, one 4-byte packet takes ~4.2 ms to transmit. The HAL document proposes staggering across cycles or increasing baud rate.

**Impact on timing budget**: If motors update at 50 Hz (every 20 ms), each cycle only needs to send 1 motor command (~4.2 ms), which fits within a 10 ms HAL update slot. But this must be explicitly modeled in the executor's timing analysis.

The safety/RT design's rate-monotonic analysis (Section 8.4) gives the DriveNode a WCET of 80 us at 50 Hz. But this budget does not include the blocking serial TX time of 4.2 ms per motor command. If the serial TX is blocking (which SoftwareSerial is), the actual WCET is ~4200 us, not 80 us.
...
| DomeNode | 60 us | **4200 us** (1 Sabertooth packet) |
| ServoNode | 40 us | **400-500 us** (Maestro batched command) |
| AudioNode | 30 us | **50-200 us** (MP3Trigger serial) |
