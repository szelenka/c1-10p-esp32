...
public:
    ...
    virtual void stop()
    {
        DOME_DEBUG_PRINTLN("[DomeDrive:stop] STOP!");
        fMotorStopped = true;
        fDrive = 0;
        fAutoDrive = 0;
        fMoving = false;
    ...
    virtual void animate(int xAxis, int speedModifier)
    {
        // if (fDomeStick.isConnected())
        // {
        ...
        {
            motionVector(xAxis, fSpeedModifier);
        }
    }
...
protected:
    ...
    void motionVector(int xAxis, float speedModifier)
    {
        // DEBUG_PRINTF(
        //     "[DomeDrive::motionVector] axis X: %4d, speedModifier: %4d\n",
        //     xAxis, speedModifier
        // );
        fWasConnected = true;
        int axisScale = 512;
        ...
        if (currentMillis - fLastCommand > fSerialLatency)
        {
            ...
            auto m = (float)(xAxis + axisScale) / (axisScale - 0.5f) - 1.0f;
            ...
            DEBUG_PRINTF(
                "target_left: %2.6f, throttle: %2.6f\n", 
                m, drive_mod);
