...
public:
    /*
        ...
        Each leg consists of two revolute joints (allowing rotation around a fixed axis) and one spherical joint
       (allowing rotation in any direction)

        Parallel Manipulator means the joint(s) are connected with multiple legs which move from a single platform in
       parallel
    ...
    std::array<float, 3> getLegAngles(float nx, float ny, float hz) {
        ...
        for (int i = 0; i < 3; ++i) {
            if (i == 0) {
                ...
                // law of cosines
                theta2 = std::acos((std::pow(mag, 2) + std::pow(f, 2) - std::pow(g, 2)) / (2 * mag * f));
            } else if (i == 1) {
                // Leg B
                ...
                theta1 = std::acos((std::sqrt(3) * x + y) / (-2 * mag));
                // law of cosines
                theta2 = std::acos((std::pow(mag, 2) + std::pow(f, 2) - std::pow(g, 2)) / (2 * mag * f));
            } else if (i == 2) {
                // Leg C
