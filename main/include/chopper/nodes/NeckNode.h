...
public:
     ...
     */
    NeckNode(dome::RSSMechanism* mechanism,
             const char* input_topic,
             const char* servo_topic,
             uint8_t servo_id_a = 0,
             uint8_t servo_id_b = 1,
             uint8_t servo_id_c = 2)
        : PublishingNode("neck")
        , mechanism_(mechanism)
        , input_topic_(input_topic)
        , servo_topic_(servo_topic)
    {
...
private:
    void onControllerInput(const messages::ControllerInput& input) {
        if (!mechanism_ || !servo_pub_) return;

        // Use a simple monotonic time proxy from the input processing
        uint64_t now_ms = last_time_ms_;

        // Handle enable/disable toggle (thumb_l button)
        if (input.button_thumb_l && !last_thumb_l_) {
            mechanism_->setEnabled(!mechanism_->isEnabled(), now_ms);
        }
        last_thumb_l_ = input.button_thumb_l;

        // Height adjust via L1/R1
        if (input.button_l1) mechanism_->decrementHeight(1.0f);
        if (input.button_r1) mechanism_->incrementHeight(1.0f);

        // Run IK: joystick slew values → PWM
        auto pwm = mechanism_->getLegPWMFromJoystick(
            input.axis_x_slew, input.axis_y_slew, now_ms);

        // Publish servo commands for each leg
        for (int i = 0; i < 3; ++i) {
            if (pwm[i] == 0) continue;  // disabled → skip
            messages::ServoCommand cmd;
            cmd.servo_id = servo_ids_[i];
