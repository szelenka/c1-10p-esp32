...
public:
    TelemetryNode(core::Executor* executor,
    ...
    bool initialize() override { return executor_ && safety_ && service_; }
    ...
    void process(uint64_t now) override {
        ...
        snap.degradation_mode = static_cast<uint8_t>(
            safety_->getDegradationManager().getCurrentMode());
    ...
    double getUpdateFrequency() const override {
        return hz_;
    }
