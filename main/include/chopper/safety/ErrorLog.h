...
public:
    ...
    static void setInstance(ErrorLog* inst) {
        test_instance_ = inst;
    }

    /// Get the active instance (test-injected or singleton).
    static ErrorLog& getActive() {
        return test_instance_ ? *test_instance_ : getInstance();
    }
    ...
    uint32_t getTotalCount() const {
        return static_cast<uint32_t>(total_count_.load(std::memory_order_relaxed));
    }

    /// Get count by severity level.
    uint32_t getCountBySeverity(Severity sev) const {
        if (sev > FATAL) return 0;
    ...
    void clear() {
        ...
        for (size_t i = 0; i < 5; ++i) {
            severity_counts_[i].store(0, std::memory_order_relaxed);
        }
