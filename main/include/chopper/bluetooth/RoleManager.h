...
struct RoleAssignmentPolicy {
     ...
     */
    ControllerRole (*assign)(
        const MacAddress& mac,
        uint16_t ctlType,
        const ControllerSlot* slots,
        uint8_t slotCount,
        void* context);
...
public:
    ...
    RoleAssignmentPolicy getPolicy() {
        return RoleAssignmentPolicy{
            "MacBased",
            &MacBasedPolicy::assignImpl,
            this
        };
    }

private:
    static ControllerRole assignImpl(
        const MacAddress& mac, uint16_t /*ctlType*/,
        const ControllerSlot* slots, uint8_t slotCount, void* ctx)
    {
...
public:
    ...
    FirstAvailablePolicy() : m_entryCount(0) {
        memset(m_preferred, 0, sizeof(m_preferred));
    }
    ...
    RoleAssignmentPolicy getPolicy() {
        return RoleAssignmentPolicy{
            "FirstAvailable",
            &FirstAvailablePolicy::assignImpl,
            this
        };
    }
...
private:
        ...
        const ControllerSlot* slots, uint8_t slotCount, void* ctx)
    {
        ...
        for (uint8_t r = 0; r < kRoleCount; r++) {
            if (!isRoleTaken(kAllRoles[r], slots, slotCount)) {
                return kAllRoles[r];
            }
        }
...
public:
    ...
    const char* getPolicyName() const { return m_policy.name; }
    ...
    ControllerRole onControllerAdded(uint8_t slotIndex, uint64_t now_ms) {
        if (!m_slots || slotIndex >= m_slotCount) {
            return ControllerRole::UNASSIGNED;
        }
        ...
        if (slot.canReclaimPreviousRole(now_ms)) {
            ...
            if (!isRoleTaken(prev, slotIndex)) {
                ...
                ESP_LOGI(kTag, "Slot %d reclaimed previous role %s",
                         slotIndex, roleToString(prev));
                return prev;
            }
        }
        ...
        if (m_policy.assign) {
            ControllerRole assigned = m_policy.assign(
                slot.mac, slot.controller_type, m_slots, m_slotCount, m_policy.context);
            slot.role = assigned;
        } else {
    ...
    bool assignRole(uint8_t slotIndex, ControllerRole role) {
        if (!m_slots || slotIndex >= m_slotCount) return false;
        if (role != ControllerRole::UNASSIGNED && isRoleTaken(role, slotIndex)) {
            ESP_LOGW(kTag, "Cannot assign %s to slot %d: role occupied",
                     roleToString(role), slotIndex);
            return false;
        }
        m_slots[slotIndex].role = role;
        ESP_LOGI(kTag, "Slot %d manually assigned role %s",
                 slotIndex, roleToString(role));
        return true;
    }
    ...
    bool swapRoles(uint8_t slotA, uint8_t slotB) {
        if (!m_slots || slotA >= m_slotCount || slotB >= m_slotCount) return false;
        if (!m_slots[slotA].isActive() || !m_slots[slotB].isActive()) return false;

        ControllerRole temp = m_slots[slotA].role;
        m_slots[slotA].role = m_slots[slotB].role;
        m_slots[slotB].role = temp;

        ESP_LOGI(kTag, "Swapped roles: slot %d=%s, slot %d=%s",
                 slotA, roleToString(m_slots[slotA].role),
                 slotB, roleToString(m_slots[slotB].role));
        return true;
    }
...
private:
    ...
    bool isRoleTaken(ControllerRole role, uint8_t excludeSlot) const {
        for (uint8_t i = 0; i < m_slotCount; i++) {
