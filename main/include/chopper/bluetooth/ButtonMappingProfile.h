...
struct ButtonMappingProfile {
    ...
    [[nodiscard]] float normalizeAxis(int32_t raw) const {
        // Apply deadband
...
inline const ButtonMappingProfile* findProfile(uint16_t controllerType) {
    for (const auto& kBuiltinProfile : kBuiltinProfiles) {
        if (kBuiltinProfile.controller_type == controllerType) {
            return &kBuiltinProfile;
        }
    }
